...
function sortedListToBST(head: ListNode | null): TreeNode | null {
  ...
  /**
   * The nomenclature here was used to mantain an standard, over
   * inumerous examples used `z`, `y` and `T` as the default terminology
   * so I thought of keeping the same names.
   * */
  ...
  function balanceTree(root: TreeNode): TreeNode {
    ...
    // The tree is unbalaced ("right-heavy") and
    // the unbalanced node is a "left" child of the parent Node (case "Left Left")
    if (balanceFactor > 1 && leftSubtreeBalanceFactor > 0) {
      return rightRotate(root);
    }

    // The tree is unbalanced ("right-heavy") and
    // the unbalanced node is a "right" child of the parent Node (case "Left Right")
    if (balanceFactor > 1 && leftSubtreeBalanceFactor < 0) {
      root.left = leftRotate(root.left as TreeNode);
      return rightRotate(root);
    }

    // The tree is unbalanced ("left-heavy") and
    // the unbalanced node is a "right" child of the parent Node (case "Right Left")
